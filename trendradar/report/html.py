...
def render_html_content(
    ...
    *,
    ...
    # Use provided time function or default datetime.now
    if get_time_func:
        now = get_time_func()
    else:
        now = datetime.now()
    ...
    # Generate hot words statistics part HTML
    stats_html = ""
    if report_data["stats"]:
        ...
        for i, stat in enumerate(report_data["stats"], 1):
            ...
            # Handle news titles under each word group, number each news
            for j, title_data in enumerate(stat["titles"], 1):
                ...
                if time_display:
                    # Simplify time display format, replace wave with ~
                    simplified_time = (
                ...
                # Handle appearance count
                count_info = title_data.get("count", 1)
                if count_info > 1:
                    stats_html += f'<span class="count-info">{count_info} hits</span>'
    ...
    # Generate new news section HTML
    new_titles_html = ""
    if report_data["new_titles"]:
        new_titles_html += f"""
                <div class="new-section">
                    <div class="new-section-title">New Trending Items (Total {report_data['total_new_count']})</div>"""
        ...
        for source_data in report_data["new_titles"]:
            ...
            # Number new news items too
            for idx, title_data in enumerate(source_data["titles"], 1):
                ranks = title_data.get("ranks", [])

                # Handle new news rank display
                rank_class = ""
                if ranks:
                    min_rank = min(ranks)
                    if min_rank <= 3:
    ...
    # Decide content order based on config
    if reverse_content_order:
        # New trending first, hot words stats second
        html += new_titles_html + stats_html
    else:
        # Default: Hot words stats first, new trending second
        ...
        <script>
            async function saveAsImage() {
                ...
                try {
                    ...
                    // Wait again to ensure buttons are completely hidden
                    ...
                    const now = new Date();
                    const filename = `TrendRadar_Report_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.png`;
            ...
            async function saveAsMultipleImages() {
                ...
                try {
                    ...
                    // 下载所有图片
                    const now = new Date();
                    const baseFilename = `TrendRadar_Report_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
